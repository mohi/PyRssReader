...
class CmdlineParser(argparse.ArgumentParser):
    ...
    def __inferConverter(self):
        if self.__args.convert:
            ...
            self.__args.convert_info = [
                CmdlineParser.inferConvert(txt) for txt in converters_info
            ]
        else:
            raise
    ...
    def __inferInput(self):
        ...
        if self.__args.input:
            if self.__args.input.endswith('.txt'):
                self.__args.input_info = self.__InputInfo(
                    InputType.FILE, self.__args.input)
            else:
                self.__args.input_info = self.__InputInfo(
                    InputType.URL, self.__args.input)
        else:
            raise
    ...
    def __inferOutput(self):
        ...
        if self.__args.output and self.__args.output.endswith('.txt'):
            self.__args.output_info = self.__OutputInfo(
                OutputType.FILE, self.__args.output)
        else:
            self.__args.output_info = self.__OutputInfo(
